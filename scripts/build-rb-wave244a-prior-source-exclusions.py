#!/usr/bin/env python3
"""Snapshot all structured source URLs and snapshot hashes before Wave244A."""
...
OUTPUT = ROOT / "docs/audits/generated/rb-wave244a-prior-source-exclusions.json"
URL_KEYS = {"source_url"}
HASH_KEYS = {"source_snapshot_sha256", "snapshot_sha256", "source_sha256"}
...
def main() -> None:
    ...
    candidates = sorted({path for path in candidates if "wave244a" not in path.name.lower()})
    ...
    for path in candidates:
        ...
        try:
            ...
            continue
        before = (len(urls), len(hashes))
        walk(payload, origin, urls, hashes)
    ...
    payload = {
        "schema_version": 1,
        "wave": "wave244a_prior_source_exclusions",
