...
cleanup() {
  docker compose exec -T -e RESTORE_DB="$restore_db" -e ARCHIVE_FILE="$archive_file" postgres sh -ceu '
...
cat "$POSTGRES_BACKUP_FILE" | docker compose exec -T -e ARCHIVE_FILE="$archive_file" postgres sh -ceu '
  umask 077
  cat > "$ARCHIVE_FILE"
'
