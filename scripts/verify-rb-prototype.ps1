...
if ($Path.Count -eq 0) {
    $prototypesDir = Join-Path $PSScriptRoot '..\docs\prototypes'
    $Path = @(Get-ChildItem -LiteralPath $prototypesDir -Filter 'rb-b2b-*.html' | Sort-Object Name | ForEach-Object { $_.FullName })
}

if ($Path.Count -eq 0) {
    Write-Error 'No RB HTML prototype files were found to verify.'
    exit 1
}
...
function Require-Match {
    ...
    if (-not [regex]::IsMatch($script:html, $Pattern, $Options)) {
...
function Forbid-Match {
    ...
    if ([regex]::IsMatch($script:html, $Pattern, $Options)) {
        $script:errors.Add($Message)
    }
}
...
function Require-QuoteFormControl {
    ...
    if ($null -eq $control -or -not $control.Success) {
        $script:errors.Add("Quote form is missing the '$Name' control.")
        return $null
    }
    ...
    if ($ExpectedTag -and $tag -ne $ExpectedTag) {
        $script:errors.Add("Quote form control '$Name' must use <$ExpectedTag>.")
    }

    $type = Get-AttributeValue -Element $element -Name 'type'
    if ($Visible -and $type -eq 'hidden') {
        $script:errors.Add("Quote form control '$Name' must be visible, not hidden.")
    }

    if ($Required -and $element -notmatch '(?i)\brequired(?:\s|=|/|>)') {
        $script:errors.Add("Quote form control '$Name' must be required by the API contract.")
    }

    if ($InputType -and $type -ne $InputType) {
        $script:errors.Add("Quote form control '$Name' must use type='$InputType'.")
    }
...
function Require-HiddenControl {
    ...
    if ((Get-AttributeValue -Element $control.Value -Name 'type') -ne 'hidden') {
        $script:errors.Add("Quote form control '$Name' must be hidden technical data.")
    }
...
function Require-JsonHiddenControl {
    ...
    if ([string]::IsNullOrWhiteSpace($value) -or -not $value.TrimStart().StartsWith($ExpectedPrefix)) {
        $script:errors.Add("Quote form control '$Name' must contain a JSON $ExpectedDataType representation.")
        return
    }
    ...
    try {
        ...
        $script:errors.Add("Quote form control '$Name' contains invalid JSON.")
    }

    if ((Get-AttributeValue -Element $control.Value -Name 'data-json') -ne $ExpectedDataType) {
        $script:errors.Add("Quote form control '$Name' must declare data-json='$ExpectedDataType'.")
    }
}
...
foreach ($singlePath in $Path) {
    $resolvedPath = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($singlePath)
    ...
    $script:html = [System.IO.File]::ReadAllText($resolvedPath, [System.Text.UTF8Encoding]::new($false))
    $script:errors = [System.Collections.Generic.List[string]]::new()
    $errors = $script:errors
    $html = $script:html

    # A prototype file is the catalog/listing template when it renders the filter rail.
    $isCatalogPage = [regex]::IsMatch($html, '\bid\s*=\s*["'']filters["'']', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
    ...
    if (-not $quoteFormMatch.Success) {
        ...
        $script:quoteFormHtml = ''
        $quoteFormOpen = ''
    }
    else {
        $script:quoteFormHtml = $quoteFormMatch.Value
